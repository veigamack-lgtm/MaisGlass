...
Scripts usados na validação das rodadas 3–7 do módulo Orçamentos. Não fazem
parte da suíte Node (`node test/test.js`); exigem Chromium via Playwright e um
...
python3 -m http.server 8765        # na raiz do projeto (index.html); ou defina GM_URL
...
Os mesmos ciclos de interface dos achados dos pareceres nº 5 e 6 também estão em
`test/ui.js` (jsdom, sem navegador, timers e `confirm()` controlados) — veja o
README principal.
