 ...
 * (A4, 1 página) na pasta temporária do sistema. Assert-based. */
const fs = require('fs');
const os = require('os');
...
executar(async () => {
  ...
  igual('botões "?" na aba Orçamentos', await p.locator('#tab-orcamentos button.help').count(), 14);
  ...
  igual('proposta com 3 itens', await p.locator('#proposta tbody tr').count(), 4);
  await p.emulateMedia({ media: 'print' });
  const arq = os.tmpdir() + '/proposta.pdf';
  await p.pdf({ path: arq, format: 'A4', printBackground: true });
  const pdf = fs.readFileSync(arq);
  igual('PDF gerado com 1 página', [pdf.length > 10000, (pdf.toString('latin1').match(/\/Type\s*\/Page[^s]/g) || []).length], [true, 1]);
