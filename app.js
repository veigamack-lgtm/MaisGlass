...
(function () {
  ...
  function migrar(cfg) { return CALC.migrarConfig(cfg, DEF.config); }
  function validarTermosCfg(c) { return window.GM_TERMOS ? GM_TERMOS.validarTermos(c && c.proposta && c.proposta.termos) : []; }   // ver calc.js → migrarConfig (v2/v3, FCP, empresa MG, DIFAL derivado)
  /* Migra + valida; lança Error com a lista de problemas. */
  function normalizarConfig(cfg) {
    var c = migrar(cfg);
    var erros = CALC.validarConfig(c).concat(validarTermosCfg(c));
  ...
  var AJUDA = {
    ...
    orc: {
      nome: { oque: 'Razão social ou nome do cliente. Aparece na lista de orçamentos e no campo "À:" da proposta impressa.', muda: 'Só identificação — não altera nenhum cálculo.', dica: 'Use o nome pelo qual você procura o cliente depois; a obra tem campo próprio mais abaixo.' },
      contato: { oque: 'Pessoa de contato do cliente (sai como "A/C:" na proposta).', muda: 'Só aparece na proposta impressa.', dica: 'Coloque quem aprova o orçamento do lado do cliente.' },
      documento: { oque: 'CNPJ (empresa) ou CPF (pessoa física) do cliente.', muda: 'Só aparece na proposta impressa. Não muda o cálculo: o que define os impostos é o campo "Destinatário".', dica: 'Construtora com CNPJ continua sendo consumidor final se não revende nem industrializa o vidro.' },
      enderecoEntrega: { oque: 'Endereço onde o vidro será entregue, se for diferente do endereço do cliente.', muda: 'Só aparece na proposta impressa (e na cláusula de frete, quando o frete está incluso).', dica: 'A UF de entrega é a que vale para DIFAL e FCP: confira se bate com o campo "UF de destino".' },
      obra: { oque: 'Nome da obra ou empreendimento.', muda: 'Só aparece na proposta impressa.', dica: 'Ajuda o cliente a encaminhar a proposta internamente.' },
      ...
      observacoes: { oque: 'Observações específicas deste orçamento. Saem logo abaixo do quadro de itens.', muda: 'Só a proposta; o padrão vem de Configurações → Padrões da proposta. Garantia, entrega, reajuste, impostos etc. já saem nos termos e condições (Configurações → Termos e condições da proposta).', dica: 'Escreva o que é particular desta venda (ex.: "não inclui ART, andaime, retirada de vidros antigos").' },
      status: { oque: 'Etapa do orçamento: rascunho, enviado ao cliente, aprovado ou perdido.', muda: 'Enviado e aprovado travam cabeçalho, itens e custos e congelam na proposta os dados da empresa, o consultor (quem emitiu) e os termos e condições. Para alterar, "Nova revisão" cria a rev. seguinte como rascunho.', dica: 'Marque "Enviado" no momento em que mandar o PDF ao cliente.' }
    }
  };
  ...
  function montarAjuda() {
    document.querySelectorAll('#tab-calc .field, #tab-revenda .field, #tab-orcamentos .field').forEach(function (f) {
      var ctl = f.querySelector('input[id], select[id], textarea[id]'), label = f.querySelector('label');
      if (!ctl || !label) return;
  ...
  var VIZ_CORES = ['#172329', '#536169', '#7A8990', '#A8874F', '#B0643F', '#8E6FA0'];   // paleta da marca: metal escuro → metal claro, bronze, terracota
  var VIZ_LUCRO = '#1F7A5A';   // lucro sempre em verde, em qualquer gráfico
  ...
  function renderGraficoGenerico(prefix, total, custoTotal, lucro, partes) {
    ...
    partes.forEach(function (p, i) {
      var cor = p.chave === 'lucro' || /^Lucro/.test(p.nome) ? VIZ_LUCRO : VIZ_CORES[i % VIZ_CORES.length];
      var frac = total > 0 && p.valor > 0 ? p.valor / total : 0;
      if (frac > 0) {
        var seg = el('div', { class: 'viz-seg', style: 'flex:' + frac + ' 1 0; background:' + cor });
      ...
      leg.appendChild(el('div', { class: 'item' }, [
        el('span', { class: 'sw', style: 'background:' + cor }),
  ...
  function renderConfig() {
    ...
    document.querySelectorAll('[data-cfg]').forEach(function (inp) {
      ...
      else inp.value = v === undefined || v === null ? '' : v;   // campos novos (empresa.ie, empresa.site) podem não existir em configurações antigas
    });
    atualizarFora();
    renderClasses();
    renderProdutos();
    ...
    renderFcp();
    renderTermosConfig();
    if (usuarioAtual && !ehAdmin()) aplicarPermissoes();   // tabelas recriadas acima também ficam só leitura
  }
  /* Termos e condições da proposta: um campo por cláusula; o que difere do padrão fica em draft.proposta.termos[id] */
  function renderTermosConfig() {
    var box = $('cfg-termos'); if (!box || !window.GM_TERMOS) return;
    box.innerHTML = '';
    var admin = !usuarioAtual || ehAdmin();
    GM_TERMOS.CLAUSULAS.forEach(function (c) {
      var atual = GM_TERMOS.textoDe(draft, c.id);
      var personalizado = atual !== c.texto;
      var ta = el('textarea', { rows: String(Math.min(12, Math.max(c.cabecalho ? 1 : 2, Math.ceil(atual.length / 95) + (atual.match(/\n/g) || []).length))) });
      ta.value = atual; ta.setAttribute('data-termo', c.id); ta.disabled = !admin;
      if (!c.texto) ta.placeholder = 'Opcional — texto acrescentado depois da parte automática';
      var tag = el('span', { class: 'tag custom' + (personalizado ? '' : ' hidden'), text: 'personalizado' });
      var restaurar = el('button', { type: 'button', class: 'btn small' + (personalizado ? '' : ' hidden'), text: 'Restaurar padrão' });
      function aplicar(valor) {
        if (!draft.proposta || typeof draft.proposta !== 'object') draft.proposta = {};
        var t = draft.proposta.termos && typeof draft.proposta.termos === 'object' ? draft.proposta.termos : {};
        if (valor === c.texto) delete t[c.id]; else t[c.id] = valor;
        if (Object.keys(t).length) draft.proposta.termos = t; else delete draft.proposta.termos;
        var pers = valor !== c.texto; tag.classList.toggle('hidden', !pers); restaurar.classList.toggle('hidden', !pers || !admin);
      }
      ta.addEventListener('input', function () { aplicar(ta.value); });
      restaurar.addEventListener('click', function () { ta.value = c.texto; aplicar(c.texto); $('tab-config').dispatchEvent(new Event('change', { bubbles: true })); });
      if (!admin) restaurar.classList.add('hidden');
      box.appendChild(el('div', { class: 'termo' }, [
        el('div', { class: 'termo-top' }, [el('b', { text: c.titulo }), el('span', {}, [tag, document.createTextNode(' '), restaurar])]),
        c.auto ? el('small', { text: 'Automático: ' + c.auto }) : null,
        ta
      ].filter(Boolean)));
    });
  }
    ...
    var tbl = el('table', {}, [
      ...
      el('tbody', {}, Object.keys(fcp).sort().map(function (uf) {
        return el('tr', {}, [el('td', { text: uf }),
          inputCell(fcp[uf] === null || fcp[uf] === undefined ? '' : Math.round(fcp[uf] * 1e6) / 1e4, function (v) { fcp[uf] = v === '' ? null : pctDe(v); }, { step: '0.5', placeholder: 'não confirmado' })]);
      }))
    ]);
  ...
  /* % digitado → fração sem ruído de ponto flutuante (0,65 / 100 = 0.006500000000000001 → 0.0065) */
  function pctDe(v) { return v === '' || v === null || v === undefined ? NaN : Number((Number(v) / 100).toPrecision(12)); }
  function lerCamposSimples() {
    document.querySelectorAll('[data-cfg]').forEach(function (inp) {
      var t = inp.getAttribute('data-type'), v;
      if (t === 'text') v = inp.value;
      else if (t === 'bool') v = inp.value === '1';
      else { v = inp.value === '' ? NaN : Number(inp.value); if (t === 'pct') v = pctDe(inp.value); }
    ...
    var tbl = el('table', {}, [
      ...
      el('tbody', {}, Object.keys(draft.classes).map(function (k) {
        ...
        return el('tr', {}, [
          ...
          inputCell(c.ii * 100, function (v) { c.ii = pctDe(v); }),
          inputCell(c.ipi * 100, function (v) { c.ipi = pctDe(v); }),
          inputCell(c.pis * 100, function (v) { c.pis = pctDe(v); }),
          inputCell(c.cofins * 100, function (v) { c.cofins = pctDe(v); })
        ]);
      }))
    ]);
    ...
    var tbl = el('table', {}, [
      ...
      el('tbody', {}, Object.keys(draft.cartao.mdr).map(function (b) {
        ...
        return el('tr', {}, [el('td', { text: b })].concat([0, 1, 2].map(function (i) {
          return inputCell(Math.round(f[i] * 1e6) / 1e4, function (v) { f[i] = pctDe(v); }, { step: '0.01' });
        })));
      }))
    ]);
    ...
    var tbl = el('table', {}, [
      ...
      el('tbody', {}, Object.keys(draft.difal).sort().map(function (uf) {
        ...
        return el('tr', {}, [
          ...
          inputCell(Math.round(d[0] * 1e6) / 1e4, function (v) { d[0] = pctDe(v); CALC.derivarDifal(draft); renderDifal(); }, { step: '0.5' }),
  ...
  function validarDraft() {
    var erros = CALC.validarConfig(draft).concat(validarTermosCfg(draft));
  ...
  /* Número da proposta: o mesmo em todas as revisões (vem da 1ª revisão; gravado em "numero" ao criar a revisão seguinte) */
  function numeroProposta(o) {
    if (typeof o.numero === 'string' && o.numero) return o.numero;
    var atual = o, vistos = {};
    while (atual && atual.revisaoDe && !vistos[atual.id]) {
      vistos[atual.id] = true;
      var ant = orcamentos.filter(function (x) { return x.id === atual.revisaoDe; })[0];
      if (!ant) break;
      if (typeof ant.numero === 'string' && ant.numero) return ant.numero;
      atual = ant;
    }
    return atual.id.slice(-6).toUpperCase();
  }
  /* emitida antes dos termos e condições (sem textos congelados): sai como foi enviada — número e formato antigos */
  function emitidaLegado(o) { return ORC.emitido(o) && !(window.GM_TERMOS && GM_TERMOS.textosValidos(o.textosProposta)); }
  function numeroLegado(o) { return o.id.slice(-6).toUpperCase() + (o.revisao > 1 ? '-r' + o.revisao : ''); }
  function numeroImpresso(o) { return emitidaLegado(o) && !o.numero ? numeroLegado(o) : numeroProposta(o); }
  function orcNumero(o) { return emitidaLegado(o) && !o.numero ? numeroLegado(o) : numeroProposta(o) + (o.revisao > 1 ? '-r' + o.revisao : ''); }
  ...
  /* O que muda os números DESTE orçamento: empresa e textos da proposta não entram; da lista de materiais só contam os
   * usados pelos itens de importação direta que ainda existem em Configurações (material novo ou removido, que nenhum
   * item usa ou que não dá para atualizar, não pede "Recalcular"). */
  function temChave(o, k) { return !!o && typeof o === 'object' && Object.prototype.hasOwnProperty.call(o, k); }
  function materiaisUsados(o) {
    var nomes = [];
    o.itens.forEach(function (it) { if (it.origem === 'importacao' && it.inputs && typeof it.inputs.produto === 'string' && nomes.indexOf(it.inputs.produto) < 0) nomes.push(it.inputs.produto); });
    return nomes;
  }
  function configCalculo(c, nomes) {
    var x = Object.assign({}, c); delete x.empresa; delete x.proposta;
    var prods = (Array.isArray(c.produtos) ? c.produtos : []).filter(function (p) { return p && nomes.indexOf(p.nome) >= 0; })
      .sort(function (a, b) { return a.nome < b.nome ? -1 : 1; });
    var cls = {};
    prods.forEach(function (p) { if (temChave(c.classes, p.classe)) cls[p.classe] = c.classes[p.classe]; });
    x.produtos = prods; x.classes = cls;
    return JSON.stringify(x, function (k, v) { return typeof v === 'number' && isFinite(v) ? Number(v.toPrecision(12)) : v; });
  }
  function configDifereDoSnapshot(o) {
    var nomes = materiaisUsados(o).filter(function (n) { return !!CALC.findProduto(config, n); });
    return configCalculo(o.configSnapshot, nomes) !== configCalculo(config, nomes);
  }
  /* Copia um material (e a classe fiscal dele, se faltar) de uma configuração para outra; devolve null se não der. */
  function comMaterial(destino, origemCfg, nome) {
    var prod = CALC.findProduto(origemCfg, nome);
    if (!prod) return null;
    var novo = clone(destino), classeNova = false;
    novo.produtos = (Array.isArray(novo.produtos) ? novo.produtos : []).concat([clone(prod)]);
    if (!temChave(novo.classes, prod.classe)) {
      if (!temChave(origemCfg.classes, prod.classe)) return null;
      novo.classes = Object.assign({}, novo.classes); novo.classes[prod.classe] = clone(origemCfg.classes[prod.classe]); classeNova = true;
    }
    return CALC.validarConfig(novo).length ? null : { cfg: novo, classeNova: classeNova, produto: prod };
  }
  /* Material cadastrado em Configurações DEPOIS de o orçamento ser criado não existe na configuração congelada dele
   * ("Produto não encontrado"). Ao adicionar/substituir um item com esse material, o cadastro dele (custo, capacidade,
   * classe fiscal se faltar) é copiado da configuração atual para a congelada do orçamento; dólar, alíquotas, cartão,
   * premissas e os demais materiais continuam congelados — nenhum item existente muda. */
  function snapshotParaItem(o, origem, inputs) {
    var snap = o.configSnapshot;
    if (origem !== 'importacao' || !inputs || typeof inputs.produto !== 'string' || CALC.findProduto(snap, inputs.produto)) return { cfg: snap, incluido: null };
    var r = comMaterial(snap, config, inputs.produto);
    return r ? { cfg: r.cfg, incluido: { produto: r.produto.nome, classe: r.classeNova ? r.produto.classe : null } } : { cfg: snap, incluido: null };
  }
  ...
  function renderCabecalhoOrc() {
    ...
    CAMPOS_CLIENTE.forEach(function (k) { $('o-' + k).value = typeof c[k] === 'string' ? c[k] : ''; });
  ...
  function aplicarTravaOrc() {
    ...
    document.querySelectorAll('#orc-cabecalho input, #orc-cabecalho select, #orc-cabecalho textarea').forEach(function (i) {
  ...
  /* campos do cliente que só vão para a proposta impressa (não mudam o cálculo) */
  ...
  function lerCabecalhoTexto() {
    ...
    CAMPOS_CLIENTE.forEach(function (k) { var v = $('o-' + k).value.trim(); if (v || typeof orc.cliente[k] === 'string') orc.cliente[k] = v; });
  ...
  function onStatusOrc() {
    ...
    if (!ORC.emitido(orc) && novo !== 'rascunho') {
      ...
      orc.numero = numeroProposta(orc);
      orc.consultor = consultorAtual();
      orc.textosProposta = montarTextosProposta(orc, config);
    }
    orc.status = novo;
  ...
  function adicionarItem(origem) {
    ...
    var c, base = snapshotParaItem(orc, origem, inputs);
    try { c = ORC.calcularItem(base.cfg, cab, item); }
    catch (e) { infoAdd(prefix, 'Não foi possível adicionar: ' + e.message, 'warn'); return; }
    if (base.incluido) orc.configSnapshot = base.cfg;   // só depois do cálculo dar certo (nada muda se falhar)
    ...
    if (base.incluido) msg += ' O material "' + base.incluido.produto + '" foi cadastrado depois deste orçamento e foi incluído na configuração congelada dele' + (base.incluido.classe ? ' (com a classe fiscal ' + base.incluido.classe + ')' : '') + '; dólar, alíquotas e os demais itens continuam como estavam.';
  ...
  function carregarNaCalculadora(item) {
    ...
    atualizarBotoesAdd();
    if (item.origem === 'importacao' && !CALC.findProduto(config, inp.produto)) {
      // material renomeado/removido em Configurações: a lista não tem como mostrá-lo — avisa antes de substituir
      $('in-addOrcInfo').appendChild(el('small', { class: 'warn', style: 'display:block', text: 'O material "' + inp.produto + '" deste item não existe mais em Configurações (renomeado ou removido). Escolha o material na lista antes de substituir; ao recalcular o orçamento, o item mantém o cadastro congelado dele.' }));
    }
  }
  ...
  function renderOrc() {
    ...
    renderGraficoGenerico('oviz', t.totalCliente, t.custoTotalReal, c.real.lucroLiquido, c.composicao.map(function (p) {
      ...
      return { nome: p.nome, sub: sub, valor: p.valor, chave: p.chave };
    }));
  ...
  /* ---------- proposta comercial impressa (formato das propostas do mercado; textos em termos.js) ---------- */
  function consultorAtual() { return usuarioAtual ? { nome: usuarioAtual.nome || '', email: usuarioAtual.email || '' } : null; }
  ...
  /* Termos e condições com os dados do orçamento (pagamento, frete, prazo, validade, dólar, FCP); congelados na emissão */
  function txt(v) { return typeof v === 'string' ? v.trim() : ''; }
  function montarTextosProposta(o, cfg) {
    if (!window.GM_TERMOS) return { referencia: '', introducao: '', normas: '', encerramento: '', clausulas: [] };
    var cd = o.condicoes, cl = o.cliente, imp = [], perdas = [];
    o.itens.forEach(function (it, i) {
      if (it.origem === 'importacao') imp.push(pad3(i + 1));
      var pd = it.inputs ? Number(it.inputs.perda) : NaN;
      perdas.push({ item: pad3(i + 1), perda: isFinite(pd) && pd >= 0 ? pd : 0 });
    });
    var inc = cd.inclusos || {};
    return GM_TERMOS.montar(cfg, {
      pagamento: cd.pagamento, parcelas: cd.parcelas, bandeira: cd.bandeira, inclusos: { frete: !!inc.frete, instalacao: !!inc.instalacao, texto: txt(inc.texto) },
      prazoEntrega: txt(cd.prazoEntrega), validadeDias: cd.validadeDias,
      uf: cl.uf, destinatario: cl.destinatario, enderecoEntrega: txt(cl.enderecoEntrega) || txt(cl.endereco),
      fcpPendente: ORC.fcpPendente(o), itensImportacao: imp, perdas: perdas, dolar: o.configSnapshot && o.configSnapshot.dolar
    });
  }
  function renderProposta(c) {
    ...
    var legado = emitidaLegado(orc);   // emitida antes dos termos: sai como foi enviada (sem cláusulas)
    var consultor = emitido ? (orc.consultor && typeof orc.consultor === 'object' ? { nome: txt(orc.consultor.nome), email: txt(orc.consultor.email) } : null) : consultorAtual();
    var textos = legado ? { referencia: '', introducao: '', normas: '', encerramento: '', clausulas: [] } : (emitido ? orc.textosProposta : montarTextosProposta(orc, config));
    function linhaRot(rotulo, valor, cls) { return el('div', cls ? { class: cls } : {}, [el('b', { text: rotulo + ' ' }), document.createTextNode(valor)]); }
    var conteudo = el('div', { class: 'p-conteudo' });

    // topo: logo + identificação da proposta
    var id = el('div', { class: 'p-id' }, [
      el('div', { class: 'p-num' }, [el('b', { text: 'Proposta Comercial Nº: ' }), el('b', { text: numeroImpresso(orc) }), document.createTextNode('  ·  ' + dataHoraCurta(dataRef))]),
      linhaRot('Revisão:', ('0' + (orc.revisao || 1)).slice(-2)),
      consultor && (consultor.nome || consultor.email) ? linhaRot('Consultor técnico comercial:', consultor.nome || consultor.email) : null,
      emp.telefone ? linhaRot('Telefone:', emp.telefone) : null,
      (consultor && consultor.email) || emp.email ? linhaRot('E-mail:', (consultor && consultor.email) || emp.email) : null
    ].filter(Boolean));
    conteudo.appendChild(el('div', { class: 'p-topo' }, [el('img', { class: 'p-logo', src: 'marca/logo-horizontal-metal.png', alt: 'MaisGlass' }), id]));

    // cliente
    var cliente = el('div', { class: 'p-cliente' }, [linhaRot('À:', txt(cl.nome) || '—', 'inteira p-dest')]);
    function campo(rotulo, valor, inteira) { if (txt(valor)) cliente.appendChild(linhaRot(rotulo, txt(valor), inteira ? 'inteira' : '')); }
    campo('A/C:', cl.contato); campo('E-mail:', cl.email); campo('Telefone:', cl.telefone); campo('CNPJ/CPF:', cl.documento);
    campo('Endereço:', cl.endereco, true);
    if (txt(cl.enderecoEntrega) && txt(cl.enderecoEntrega) !== txt(cl.endereco)) campo('Endereço da entrega:', cl.enderecoEntrega, true);
    campo('IE:', cl.ie); cliente.appendChild(linhaRot('UF de entrega:', cl.uf));
    campo('Obra:', cl.obra); campo('Projeto:', cl.projeto);
    conteudo.appendChild(cliente);
    if (textos.referencia) conteudo.appendChild(el('p', { class: 'p-ref', text: 'REF.: ' + textos.referencia }));
    if (textos.introducao) conteudo.appendChild(el('p', { class: 'p-intro', text: textos.introducao }));

    if (!orc.itens.length) {
      conteudo.appendChild(el('p', { class: 'p-vazio', text: 'Adicione itens para montar a proposta.' }));
      p.appendChild(conteudo); return;
    }
    ...
    var linhas = c.itens.map(function (r, i) {
      ...
      return el('tr', {}, [el('td', { text: pad3(i + 1) }), el('td', { class: 'num', text: numFmt(r.quantidade, 2) }), el('td', { text: 'M2' }),
        el('td', { text: r.descricao || ORIGEM_ROTULO[r.origem] }), el('td', { class: 'num', text: numFmt(unit, 2) }), el('td', { class: 'num', text: numFmt(tot, 2) })]);
    });
    var totalImpresso = round2(c.totais.totalCliente);
    linhas.push(el('tr', { class: 'tot' }, [el('td', { colspan: '5', class: 'num', text: 'TOTAL' }), el('td', { class: 'num', text: brl(totalImpresso) })]));
    var cab = [['ITEM', ''], ['QTD', 'num'], ['UM', ''], ['DESCRIÇÃO', ''], ['VLR UNITÁRIO R$/m²' + (unitarioAprox ? ' *' : ''), 'num'], ['VLR TOTAL R$', 'num']];
    conteudo.appendChild(el('div', { class: 'table-wrap-p' }, [el('table', { class: 'p-itens' }, [
      el('thead', {}, [el('tr', {}, cab.map(function (h) { return el('th', { text: h[0], class: h[1] }); }))]), el('tbody', {}, linhas)])]));
    var notasArred = [];
    ...
    notasArred.forEach(function (n) { conteudo.appendChild(el('p', { class: 'p-nota', text: n })); });
    if (textos.normas) conteudo.appendChild(el('div', { class: 'p-normas', text: textos.normas }));
    if (txt(cd.observacoes)) conteudo.appendChild(el('p', { class: 'p-obs' }, [el('b', { text: 'OBSERVAÇÕES: ' }), document.createTextNode(txt(cd.observacoes))]));
    if (legado) {
      // como era impressa antes dos termos e condições: pagamento, entrega, incluso e nota de impostos
      var inc = cd.inclusos || {};
      var incl = [inc.frete ? 'frete' : '', inc.instalacao ? 'instalação' : '', txt(inc.texto)].filter(Boolean).join(', ');
      conteudo.appendChild(el('p', { class: 'p-obs' }, [el('b', { text: 'Pagamento: ' }), document.createTextNode(cd.pagamento === 'Parcelado' ? 'parcelado em ' + cd.parcelas + 'x no cartão ' + cd.bandeira + ' (taxa do cartão já incluída nos preços)' : 'à vista')]));
      if (txt(cd.prazoEntrega)) conteudo.appendChild(el('p', { class: 'p-obs' }, [el('b', { text: 'Entrega: ' }), document.createTextNode(txt(cd.prazoEntrega))]));
      conteudo.appendChild(el('p', { class: 'p-obs' }, [el('b', { text: 'Incluso: ' }), document.createTextNode(incl || 'somente os itens listados')]));
      var fcpL = ORC.fcpPendente(orc);
      conteudo.appendChild(el('p', { class: 'p-nota', text: (fcpL.length ? 'Preços com impostos inclusos (IPI, ICMS e DIFAL conforme o destino). O adicional estadual do Fundo de Combate à Pobreza (FCP) de ' + fcpL.join(', ') + ' será confirmado na emissão da nota fiscal e poderá ser acrescido ao valor.' : 'Preços com impostos inclusos (IPI, ICMS, DIFAL e FCP conforme o destino).') + ' Esta proposta não cobre obrigações tributárias do destinatário.' + (cd.validadeDias ? ' Validade: ' + cd.validadeDias + ' dia' + (cd.validadeDias > 1 ? 's' : '') + '.' : '') }));
      conteudo.appendChild(el('p', { class: 'p-aviso-tela', text: 'Proposta emitida antes dos termos e condições: é reimpressa como foi enviada, sem as cláusulas. Para enviar ao cliente com os termos e condições, use "Nova revisão".' }));
    }

    // termos e condições
    var termos = el('div', { class: 'p-termos' });
    textos.clausulas.forEach(function (cla) {
      termos.appendChild(el('h4', { text: cla.titulo + (/:$/.test(cla.titulo) ? '' : ':') }));
      cla.paragrafos.forEach(function (t) { termos.appendChild(el('p', { text: t })); });
    });
    conteudo.appendChild(termos);

    // encerramento e assinaturas
    if (textos.encerramento) conteudo.appendChild(el('p', { class: 'p-encerramento', text: textos.encerramento }));
    if (!legado) conteudo.appendChild(el('div', { class: 'p-assinaturas' }, [
      el('div', {}, [el('p', { text: 'Atenciosamente,' }), el('div', { class: 'linha' }), el('p', {}, [el('b', { text: emp.razaoSocial || 'MaisGlass' })]),
        emp.cnpj ? el('p', { text: 'CNPJ ' + emp.cnpj + (emp.ie ? ' · IE ' + emp.ie : '') }) : null,
        consultor && consultor.nome ? el('p', { text: consultor.nome + ' — consultor técnico comercial' }) : null].filter(Boolean)),
      el('div', {}, [el('p', { text: 'De acordo,' }), el('div', { class: 'linha' }), el('p', {}, [el('b', { text: 'Contratante' }), document.createTextNode(txt(cl.nome) ? ': ' + txt(cl.nome) : '')]),
        el('p', { text: 'Nome:' }), el('p', { text: 'CPF/RG:' }), el('p', { text: 'Data: ____/____/______' })])
    ]));

    // página: tfoot reserva o espaço do rodapé em cada folha impressa; o rodapé fica fixo no pé de cada página
    p.appendChild(el('table', { class: 'p-pagina' }, [el('tbody', {}, [el('tr', {}, [el('td', {}, [conteudo])])]), el('tfoot', {}, [el('tr', {}, [el('td', {})])])]));
    var rod = [emp.cnpj ? 'CNPJ ' + emp.cnpj : '', emp.endereco, emp.telefone, emp.email, emp.site].filter(function (x) { return x && String(x).trim(); });
    p.appendChild(el('div', { class: 'p-rodape', text: rod.join(' | ') }));
  }
  ...
  function recalcularOrc() {
    ...
    var cab = ORC.cabecalhoDe(orc), novos, cfgNova = config, mantidos = [];
    // material usado por item que foi renomeado/removido em Configurações: mantém o cadastro congelado do orçamento
    materiaisUsados(orc).forEach(function (nome) {
      if (CALC.findProduto(cfgNova, nome)) return;
      var r = comMaterial(cfgNova, orc.configSnapshot, nome);
      if (r) { cfgNova = r.cfg; mantidos.push(nome); }
    });
    try { novos = recalcularItens(cfgNova, cab); }
    catch (e) { orcStatus('Recalcular cancelado — ' + e.message + '. Nada foi alterado.', 'err'); return; }
    var antes = ORC.consolidar(orc);
    var simulado = Object.assign({}, orc, { itens: novos, premissas: ORC.premissasDe(cfgNova), configSnapshot: cfgNova, versaoMotor: ORC.VERSAO_MOTOR });
    ...
    if (mantidos.length) linhas.push('', 'Material que não existe mais em Configurações (renomeado ou removido): ' + mantidos.join(', ') + ' — mantido com o cadastro congelado do orçamento. Para trocar, use "Carregar" no item, escolha o material atual e substitua.');
    ...
    orc.itens = novos; orc.premissas = ORC.premissasDe(cfgNova); orc.configSnapshot = clone(cfgNova); orc.versaoMotor = ORC.VERSAO_MOTOR;
    renderOrc();
    if (salvarOrcAtual()) orcStatus('Itens recalculados com a configuração atual e salvos.' + (mantidos.length ? ' Material mantido do cadastro congelado: ' + mantidos.join(', ') + '.' : ''), 'ok');
  ...
  function novaRevisaoOrc() {
    ...
    n.numero = numeroProposta(orc); delete n.consultor; delete n.textosProposta;   // rascunho volta a acompanhar Configurações
  ...
  function duplicarOrc() {
    ...
    delete n.numero; delete n.consultor; delete n.textosProposta;
  ...
  function importarUmOrc(dadosArquivo, abrir) {
    ...
    try {
      ...
      var erros = ORC.validarOrcamento(o).concat(window.GM_TERMOS ? GM_TERMOS.validarCamposProposta(o) : []);
  ...
  function imprimirOrc() {
    ...
    // nome sugerido do PDF = título da página: "Proposta MaisGlass 3F9A2C rev 2 - Cliente"
    var tituloAntes = document.title;
    document.title = 'Proposta MaisGlass ' + numeroImpresso(orc) + (orc.revisao > 1 && !(emitidaLegado(orc) && !orc.numero) ? ' rev ' + orc.revisao : '') + (orc.cliente.nome ? ' - ' + orc.cliente.nome.replace(/[\\/:*?"<>|]+/g, ' ') : '');
    var imprimir = function () { window.print(); document.title = tituloAntes; };
    var logo = $('proposta').querySelector('img.p-logo');
    if (logo && !logo.complete && logo.decode) logo.decode().then(imprimir, imprimir); else imprimir();
  }
  ...
  function iniciarOrcamentos() {
    ...
    ['o-nome', 'o-contato', 'o-validade', 'o-prazo', 'o-dataPrevista', 'o-inclusoFrete', 'o-inclusoInstalacao', 'o-inclusoTexto', 'o-observacoes'].concat(CAMPOS_CLIENTE.map(function (k) { return 'o-' + k; })).forEach(function (id) {
